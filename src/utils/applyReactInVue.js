...
import { createPortal, version, ReactDOM } from 'react-dom';
...
const createReactContainer = (Component, options, wrapInstance) =>
  class applyReact extends React.Component {
    // reactDevTools
    ...
    reactPropsLinkToVueInstance(ref) {
      ...
      Object.getOwnPropertyNames(ref.__proto__)
        .filter((key) => ['constructor', 'render'].indexOf(key) < 0)
        .forEach((key) => {
          if (!wrapInstance[key]) {
            wrapInstance[key] = ref[key];
          }
      ...
      if (division) {
        if (children && children[0]) {
          return children.map((child, index) =>
            applyVueInReact(this.createSlot(child instanceof Function ? child : [child]), {
              ...options,
              ...customOptions,
              isSlots: true,
              wrapInstance,
    ...
    render() {
        ...
        if (props[i].__slot) {
          if (!props[i].reactSlot) {
            ...
            if (options.defaultSlotsFormatter && props[i].__trueChildren) {
              ...
              props[i] = options.defaultSlotsFormatter(
                props[i].__trueChildren,
                this.vueInReactCall,
                hashList,
              );
              ...
              props[i] = {
                ...applyVueInReact(this.createSlot(props[i]), {
                  ...options,
                  isSlots: true,
                  wrapInstance,
                }).render(),
        ...
        Object.keys(Component.__syncUpdateForPureReactInVue).map((key) => {
          if (finalProps[key] && typeof finalProps[key] === 'function') {
            ...
            finalProps[key] = function (...args) {
              __veauryVueWrapperRef__.__veaurySyncUpdateProps__(
                Component.__syncUpdateForPureReactInVue[key].apply(this, args),
              );
      ...
      if (options.defaultPropsFormatter) {
        finalProps = options.defaultPropsFormatter.call(
          this,
          finalProps,
          this.vueInReactCall,
          hashList,
      ...
      if (
        ...
        return <Component {...newProps} {...refInfo} />;
      }
      return (
        <FunctionComponentWrap passedProps={newProps} component={Component} {...refInfo}>
          {newProps.children}
...
export default function applyReactInVue(component, options = {}) {
  ...
  return {
    ...
    setup(props, context) {
      ...
      const injection =
        options.useInjectPropsFromWrapper || component.__veauryInjectPropsFromWrapper__;
    ...
    render() {
      ...
      const VNode = createElement(
        options.react.componentWrap,
        { ref: 'react', ...(options.react.componentWrapAttrs || {}) },
        this.VEAURY_Portals.map(({ Portal, key }) => Portal(createElement, key)),
      );
        ...
        function scopedSlotFunction(createReactSlot) {
          function getSlot(...args) {
            ...
            if (options.defaultSlotsFormatter) {
              ...
              scopeSlot = options.defaultSlotsFormatter(
                scopeSlot,
                _this.__veauryVueInReactCall__,
                hashList,
              );
              if (
                scopeSlot instanceof Array ||
                (typeof scopeSlot).indexOf('string', 'number') > -1
              ) {
                scopeSlot = [...scopeSlot];
            ...
            return applyVueInReact(
              createReactSlot(() => slotFunction.apply(this, args)),
              { ...options, isSlots: true, wrapInstance: _this },
            ).render();
          }
      ...
      __veauryMountReactComponent__(update, updateType, extraData = {}) {
        ...
        if (!update || (update && updateType?.slot)) {
          ...
          for (let i in this.$slots || {}) {
            ...
            scopedSlots[i] = this.__veauryGetScopeSlot__(
              this.$slots[i],
              hashList,
              this.$.vnode?.children?.[i],
            );
          }
        }
        ...
        if (!update) {
          ...
          if (reactWrapperRef) {
            ...
            this.reactPortal = () => createPortal(reactRootComponent, container);
