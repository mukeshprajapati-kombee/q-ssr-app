...
const VueContainer = React.forwardRef((originProps, ref) => {
  ...
  const injection =
    globalOptions.useInjectPropsFromWrapper || component.__veauryInjectPropsFromWrapper__;
...
class VueComponentLoader extends React.Component {
  ...
  createVueComponentContainer() {
    ...
    return options.vue.componentWrapHOC(
      <div {...options.vue.componentWrapAttrs} ref={this.__veauryCreateVueInstance__} key={null} />,
      nativeProps,
    );
  }
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    ...
    // update vue $data
    this.__veauryVueInstance__ &&
      Object.assign(this.__veauryVueInstance__.$data, parseVModel(props));
    return true;
  }
  ...
  __veauryCreateVueInstance__(targetElement) {
    ...
    const vueOptions = {
      data() {
        return options.isSlots
          ? { children: VueContainerInstance.__veauryCurrentVueComponent__.originVNode }
          : vueOptionsData;
      },
      created() {
          ...
          if (division) {
            if (children && children[0]) {
              return children.map((child, index) =>
                createElement(RenderReactNode, { node: child, key: child?.data?.key || index }),
              );
            }
          }
              ...
              return (...args) => {
                ...
                if (!this.getScopedSlots.__scopeSlots[i]?.component?.ctx?.__veauryReactInstance__) {
                  ...
                  if (defaultSlotsFormatter && slotFromReact) {
                    ...
                    newSlot = createElement(
                      applyReactInVue(() => scopedSlot.apply(this, args), {
                        ...options,
                        isSlots: true,
                  ...
                  Promise.resolve().then(() => {
                    newSlot?.component?.ctx?.__veauryReactInstance__?.setState({
                      children: scopedSlot.apply(this, args),
                    });
                  });
                }
      ...
      render() {
        ...
        let { component, $slots, children, class: className, style, ...lastProps } = this.$data;
        ...
        return createElement(
          filterVueComponent(VueContainerInstance.__veauryCurrentVueComponent__, this),
          {
            // ...lastProps,
            ...lastAttrs,
            ...(className || newClassName || newClassName1
    ...
    if (vueWrapperRef && document.getElementById(targetId)) {
      ...
      this.vuePortal = (createElement, key) =>
        createElement(Teleport, { to: '#' + targetId, key: targetId }, [
          createElement(Object.assign(vueOptions, { router: this._router })),
        ]);
  ...
  updateVueComponent(nextComponent) {
    ...
    if (nextComponent.__fromReactSlot) {
      this.__veauryVueInstance__.children =
        typeof nextComponent.originVNode === 'function'
          ? nextComponent.originVNode
          : () => nextComponent.originVNode;
    } else {
...
export default function applyVueInReact(component, options = {}) {
  ...
  const returnedReactComponent = React.forwardRef((props, ref) => {
    return (
      <VueContainer {...props} component={component} ref={ref} {...{ [optionsName]: options }} />
    );
  });
