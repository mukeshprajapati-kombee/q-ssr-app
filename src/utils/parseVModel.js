function createModifiers(VModels, modelKey, modifiers) {
  ...
  return (VModels[(modelKey === 'modelValue' ? 'model' : modelKey) + 'Modifiers'] =
    modifiersObject);
}
...
function setVModel(VModels, originValue, modelKey, errorFrom = 'v-model') {
  ...
  if (modelMix instanceof Array) {
    if (typeof modelMix[1] !== 'function') {
      throw Error(
        `[error:veaury] Parameter type error from '${errorFrom}', a single v-model is an array, the second element of the array must be a setter function`,
      );
    }
    ...
    throw Error(
      `[error:veaury] Parameter type error from '${errorFrom}', a single v-model is an array, such as [val, setter, argumentKey, modifiers] or [val, setter, modifiers]`,
    );
  }
}
  ...
  Object.keys(props).forEach((key) => {
    ...
    if (key === 'v-models') {
      if (typeof props[key] === 'object' && !(props[key] instanceof Array)) {
        ...
        throw Error(
          "[error:veaury] The parameter 'v-models' must be an object type, such as {[argumentKey]: singleVModel}",
        );
      }
    }
  });
